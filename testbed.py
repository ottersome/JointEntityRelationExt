...
model = CopyAttentionBoi.load_from_checkpoint(
    args.chkpnt_path,
    amount_of_relations=num_relationships,
    tokenizer=tokenizer,
...
for i, sample in enumerate(samples):
    tset_tokens[i, :] = torch.LongTensor(sample[0])
...
# estimations = model.generate(tset_tokens, attn_mask)
# Compare Estimations with Actual Target

logger.info("Evaluation Report: " f"- We have evaluated {len(samples)} samples")
for i in range(estimations.size(0)):
    # First Padding Indices
    fpi_est = torch.where(estimations[i, :] == tokenizer.pad_token_id)[0][0]
    fpi_targ = torch.where(target[i, :] == tokenizer.pad_token_id)[0][0]

    # Decode
    est_dec = tokenizer.decode(estimations[i, :fpi_est])
    est_targ = tokenizer.decode(target[i, :fpi_targ])
